...
//            cameraHelper.startPreview();
                ...
                cameraHelper.setCameraHelperCallback(new CameraHelper.OnCameraHelperCallback() {
                    ...
                    public void onPreviewFrame(byte[] data, Camera camera, int displayOrientation, int previewOrientation) {
                        try {
                            Log.i(TAG, "limeopenCamera prepareFacePassDetect 104");
                            ...
                            Log.d(TAG, "limeFile onPreviewFrame width: " + width + "  height: " + height + "   getPreviewFormat: " + parameters.getPreviewFormat());
                            ...
                            runUIThreadWithCheck(new Runnable() {
                                ...
                                public void run() {
                                    if (MainApplication.isUnLockFrame) {
                                        MainApplication.imageCache = data;
                                    }
                                    Log.d(TAG, "limeFile prepareFacePassDetect: " + 120);
                                    if (isFaceDualCamera) {
                                        Log.i(TAG, "limeopenCamera prepareFacePassDetect 118");
                                        facePassHandlerHelper.addRgbFrame(facePassImage);
                                    } else {
                                        Log.i(TAG, "limeopenCamera prepareFacePassDetect 121");
